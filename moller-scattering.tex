...
\begin{equation*}
F(\theta)=\frac{I(\theta)}{I(\pi-\alpha)},\qquad\alpha\le\theta\le\tfrac{\pi}{2}-\alpha
\end{equation*}
...
\noindent
Run ``moller-scattering-3.txt'' to draw a graph of $f(\theta)$ for $\alpha=\pi/6=30^\circ$.
...
\noindent
The following table shows the probability distribution for $30^\circ$ bins ($\alpha=\pi/6=30^\circ$).
\begin{center}
\begin{tabular}{|c|c|c|}
\hline
