\input{preamble}

\section*{Rutherford scattering 2}

Find the scattering cross section for the screened Coulomb potential
\begin{equation*}
V(r)=-\frac{Z\alpha\hbar c}{r}\exp\left(-\frac{r}{a}\right)
\end{equation*}

Let $f(\mathbf p)$ be the scattering amplitude for momentum transfer vector
$\mathbf p=\mathbf p_i-\mathbf p_f$.
The following formula is the Born approximation for $f(\mathbf p)$.
\begin{equation*}
f(\mathbf p)
=\frac{m}{2\pi\hbar^2}
\int\exp\left(\frac{i\mathbf p\cdot\mathbf r}{\hbar}\right)V(\mathbf r)\,d\mathbf r
\end{equation*}

Convert to polar coordinates where $p=|\mathbf p|$.
\begin{equation*}
f(\mathbf p)
=\frac{m}{2\pi\hbar^2}
\int_0^\infty
\int_0^\pi
\int_0^{2\pi}
\exp\left(\frac{ipr\cos\theta}{\hbar}\right)V(r,\theta,\phi)
\,r^2\sin\theta\,dr\,d\theta\,d\phi
\end{equation*}

Substitute the screened Coulomb potential
\begin{equation*}
V(r,\theta,\phi)=-\frac{Z\alpha\hbar c}{r}\exp\left(-\frac{r}{a}\right)
\end{equation*}

to obtain
\begin{equation*}
f(\mathbf p)
=-\frac{mZ\alpha c}{2\pi\hbar}
\int_0^\infty
\int_0^\pi
\int_0^{2\pi}
\exp\left(\frac{ipr\cos\theta}{\hbar}-\frac{r}{a}\right)
r\sin\theta\,dr\,d\theta\,d\phi
\end{equation*}

Integrate over $\phi$ (multiply by $2\pi$).
\begin{equation*}
f(\mathbf p)
=-\frac{mZ\alpha c}{\hbar}
\int_0^\infty
\int_0^\pi
\exp\left(\frac{ipr\cos\theta}{\hbar}-\frac{r}{a}\right)
r\sin\theta\,dr\,d\theta
\end{equation*}

Transform the integral over $\theta$ into an integral over $u$
where $u=\cos\theta$ and $du=-\sin\theta\,d\theta$.
The minus sign in $du$ is canceled by interchanging integration limits
$\cos0=1$ and $\cos\pi=-1$.
\begin{equation*}
f(\mathbf p)
=-\frac{mZ\alpha c}{\hbar}
\int_0^\infty
\int_{-1}^1
\exp\left(\frac{ipru}{\hbar}-\frac{r}{a}\right)
r\,dr\,du
\end{equation*}

Solve the integral over $u$.
\begin{equation*}
f(\mathbf p)=-\frac{mZ\alpha c}{\hbar}
\int_0^\infty
\left[\frac{\hbar}{ipr}\exp\left(\frac{ipru}{\hbar}-\frac{r}{a}\right)\right]_{u=-1}^{u=1}
r\,dr
\end{equation*}

Cancel $r$ and evaluate the limits.
\begin{equation*}
f(\mathbf p)=-\frac{mZ\alpha c}{\hbar}
\int_0^\infty
\frac{\hbar}{ip}
\left[\exp\left(\frac{ipr}{\hbar}-\frac{r}{a}\right)
-\exp\left(-\frac{ipr}{\hbar}-\frac{r}{a}\right)\right]
dr
\end{equation*}

Solve the integral over $r$.
\begin{equation*}
f(\mathbf p)=-\frac{mZ\alpha c}{\hbar}
\frac{\hbar}{ip}
\left[
\frac{1}{ip/\hbar-1/a}
\exp\left(\frac{ipr}{\hbar}-\frac{r}{a}\right)
+\frac{1}{ip/\hbar+1/a}
\exp\left(-\frac{ipr}{\hbar}-\frac{r}{a}\right)
\right]_{r=0}^{r=\infty}
\end{equation*}

Evaluate the limits. The exponentials vanish at the upper limit.
\begin{equation*}
f(\mathbf p)=-\frac{mZ\alpha c}{\hbar}
\frac{\hbar}{ip}
\left[
-\frac{1}{ip/\hbar-1/a}
-\frac{1}{ip/\hbar+1/a}
\right]
\tag{1}
\end{equation*}

Hence
\begin{equation*}
f(\mathbf p)=-\frac{2mZ\alpha\hbar c}{p^2+(\hbar/a)^2}
\end{equation*}

Note that $\mathbf p$ is momentum transfer such that
\begin{equation*}
p^2=|\mathbf p|^2=4mE(\cos\theta-1)
\end{equation*}

Hence
\begin{equation*}
f(\theta)=-\frac{2mZ\alpha\hbar c}{4mE(\cos\theta-1)+(\hbar/a)^2}
\tag{2}
\end{equation*}

Cancel $m$ in the numerator.
\begin{equation*}
f(\theta)=-\frac{2Z\alpha\hbar c}{4E(\cos\theta-1)+\frac{1}{m}(\hbar/a)^2}
\end{equation*}

The cross section is
\begin{equation*}
\frac{d\sigma}{d\Omega}=|f(\theta)|^2
=\left[\frac{2Z\alpha\hbar c}{4E(\cos\theta-1)+\frac{1}{m}(\hbar/a)^2}\right]^2
\end{equation*}

Let $a\rightarrow\infty$ to obtain the ordinary Rutherford cross section.
\begin{equation*}
\frac{d\sigma}{d\Omega}
=\left[\frac{Z\alpha\hbar c}{2E(\cos\theta-1)}\right]^2
\end{equation*}

\href{https://georgeweigt.github.io/examples/rutherford-scattering-2-demo.html}{Eigenmath script}

\end{document}
