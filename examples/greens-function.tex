...
Find the Green's function $G(\mathbf x)$ such that
\begin{equation*}
...
By linearity of differentiation we have
\begin{equation*}
...
\begin{equation*}
G(\mathbf x)
=-\frac{\exp(ikx)}{4\pi x},\quad x=|\mathbf x|
\tag{3}
\end{equation*}

Verify that (3) satisfies (1).
From Griffiths and Schroeter problem 10.8
\begin{equation*}
...
Substituting $\mathbf r$ for $\mathbf x$ we have
\begin{align*}
...
\\
&=-\frac{1}{4\pi}\biggl(
\underset{\text{see eq.~(5)}}{\nabla\frac{1}{r}\cdot\nabla e^{ikr}}
+\underset{\text{see eq.~(6)}}{\frac{1}{r}\nabla^2e^{ikr}}
+\underset{\text{see eq.~(5)}}{\nabla e^{ikr}\cdot\nabla\frac{1}{r}}
+e^{ikr}\nabla^2\frac{1}{r}
\biggr)\tag{4}
\end{align*}
...
Substitute (5) and (6) into (4) to obtain
\begin{align*}
...
Noting that $e^{ikr}=1$ at $r=0$ the $e^{ikr}$ can be discarded leaving
\begin{equation*}
