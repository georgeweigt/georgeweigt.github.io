...
$e^-+Z\rightarrow e^-+Z$ where $Z$ is an atom.
...
where $Z$ is atomic number and $q^2$ is momentum transfer such that
\begin{equation*}
...
\begin{equation*}
\langle\vert\mathcal{M}\vert^2\rangle
=\left(\frac{Z\alpha}{q^2}\right)^2\left[4m^2+2p^2(\cos\theta+1)\right]
\end{equation*}

Recall that $m\approx0.511\,\text{MeV}$.
For $m\gg p$ we can use the approximation
\begin{equation*}
4m^2+2p^2(\cos\theta+1)\approx4m^2
\end{equation*}
...
where $E$ is kinetic energy (not relativistic energy).

\bigskip
Restore $\hbar$ and $c$.
\begin{equation*}
\frac{d\sigma}{d\Omega}=\left[\frac{Z\alpha\hbar c}{2E(\cos\theta-1)}\right]^2
\end{equation*}
