...
\begin{align*}
I(\xi)
&=\frac{64E^2}{2\pi\alpha^2}\int_0^{2\pi}\int_a^\xi d\sigma\,\sin\theta\,d\theta\,d\phi
\\
&=\int_a^\xi \frac{\left(\cos^2\theta+3\right)^2}{\sin^4(\theta/2)}\,\sin\theta\,d\theta,
\qquad a\le\xi\le\pi
\end{align*}

\noindent
Angular support is limited to $a>0$ because $I(0)$ is undefined.
...
\noindent
``Count'' is the number of observed scattering events per bin.

\bigskip
\noindent
Integrate the probability density function
\begin{equation*}
f(\theta)=C^{-1}\frac{\left(\cos^2\theta+3\right)^2}{\sin^4(\theta/2)}\sin\theta
\end{equation*}
to obtain a scattering event probability per bin.
Then multiply each probability by the total number of observed counts (15773) to obtain
the following predicted counts per bin.
...
\section*{Notes on Eigenmath scripts}
