...
\begin{align*}
I(\theta)
&=\left(\frac{4s}{\alpha^2}\right)\frac{1}{2\pi}
...
\bigskip
\noindent
To compute predicted counts $\hat{y}$, integrate probability density function $f(\theta)$
over each bin and multiply by the total number of counts.
\begin{equation*}
P_k=\frac{1}{C}\int_{\arccos(x_k)}^{\arccos(x_{k+1})}
\left(
\frac{\cos^2\theta+3}{\cos\theta-1}
