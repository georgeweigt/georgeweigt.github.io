...
where
\begin{equation*}
x=\cos\theta,\quad y=\text{counts}
\end{equation*}

Use the cumulative distribution function $F(\theta)$ to compute the probability for each bin.
\begin{equation*}
P_k=F\left(\cos^{-1}(x_{k+1})\right)-F\left(\cos^{-1}(x_k)\right)
\end{equation*}

Multiply $P_k$ by total counts to obtain predicted counts $\hat{y}_k$.
\begin{equation*}
\sum_{j=1}^{12}y_j=15773,\quad \hat{y}_k=15773\,P_k
\end{equation*}
